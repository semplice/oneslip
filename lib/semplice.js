...
wss.on('connection', function(ws) {
	ws.on('message', function(message) 
	{
		...
		if (message.beginsWith("network")){ net = exec("nmcli "+ message.split("network")[1], function(err, stdout, stderr) {
		...
		if (message.beginsWith("mixer")){ exec("amixer -q sset Master "+ message.split("mixer")[1], puts); };		
	});	
});